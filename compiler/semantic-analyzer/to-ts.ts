import type { AssignmentPattern, PostfixUnaryOperator } from 'typescript';
...
export class ToTSVisitor extends BaseVisitor {
	...
	// protected writer: FileSink;

	constructor() {
		super();

		// const file = Bun.file(path.resolve(process.cwd(), 'out.b'));
		// this.writer = file.writer({ highWaterMark: 2048 * 2048 });
	}
	...
	visitSourceFile(node: SourceFileNode) {
		...
		// for (const part of this.visitEachChild(node)) {
		// 	this.writer.write(part);
		// 	this.writer.write('\n');
		// }

		// this.writer.write(`process.exit(main(process.argv))\n`);
		// this.writer.end();
		//
...
`;
	...
	visitParameter(node: ParameterNode) {
		// this.writer.write(`${node.ident.image}: ${node.type.image}`);
